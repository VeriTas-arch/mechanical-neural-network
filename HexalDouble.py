...
def pymunk_run(queue, process_num, popGame):
    ...
    print("GENERATIONS=", N_GENERATIONS, "POP_SIZE=", POP_SIZE)
    ...
    pop_pos_1 = np.zeros((POP_SIZE, node_num, 2))
    pop_pos_2 = np.zeros((POP_SIZE, node_num, 2))
    ...
    for gen in range(N_GENERATIONS):
        ...
        # exam the deformation twice
        for i in range(POP_SIZE):
            popGame._reset_game(pop[i])
            popGame.run_game(set.force_1)
            pop_pos_1[i] = [popGame.node_list[j].position for j in range(node_num)]

        for i in range(POP_SIZE):
            popGame._reset_game(pop[i])
            popGame.run_game(set.force_2)
            pop_pos_2[i] = [popGame.node_list[j].position for j in range(node_num)]

        # get the fitness of the population
        fitness = np.array(
            [
                ind_fitness
                for ind_fitness in map(EVADouble.get_fitness, pop_pos_1, pop_pos_2)
            ]
        )
        ...
        if process_num == 0:
            ...
            print(f"\nEVADouble{process_num} is processing {progress:.2f}%")
